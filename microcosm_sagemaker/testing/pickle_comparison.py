...
class IsObjectEqual(BaseMatcher):
    """
    Matcher to compare the object blob saved at filepath A with another object blob saved
    at filepath B.
    ...
    """
    def __init__(self, obj, ignore_properties=None):
    ...
    def _get_dict_structure(self, obj):
        if isinstance(obj, dict):
            return obj
        return vars(obj)


def pickle_extractor(binary):
...
def matches_with_object(obj):
    return partial(_matches_with_object, ignore_properties=[])


def _get_dir_pickles(dir, pickle_suffixes):
    return [
...
def _create_matchers(dir_pickles):
    return {
        Path(path): ExtractorMatcherPair(
            pickle_extractor,
            matches_with_object,
        )
